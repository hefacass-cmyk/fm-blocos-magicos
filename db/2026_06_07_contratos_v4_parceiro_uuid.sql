-- v4: aceitar parceiro_indicador_id (UUID) dentro do payload dados,
-- além de p_parceiro_slug (texto/slug). Mantém assinatura existente.
...
begin
  ...
  v_uuid_in := nullif(p_dados->>'parceiro_indicador_id', '');
  if v_uuid_in is not null then
    begin v_parceiro_id := v_uuid_in::uuid::text;
